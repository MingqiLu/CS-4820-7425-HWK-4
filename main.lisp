...
; (property <<-iff-not-<<=-swap (a :all b :all)
;   (== (not (<<= b a)) 
;       (<< a b)))

; (property <<-implies-<<= (a :all b :all)
;   (implies (<< a b)
;            (<<= a b)))
...
(definec all-<< (x :tl a :all) :bool
  (match x
    (() t)
    ((e . es) (and (<< e a) (all-<< es a)))))

(property less-all-<< (a :all x :tl)
  (all-<< (less a x) a))

(definec all-<<= (a :all x :tl) :bool
  (match x
    (() t)
    ((e . es) (and (<<= a e) (all-<<= a es)))))

(property notless-all-<<= (a :all x :tl)
  (all-<<= a (notless a x)))
...
(property ordered-append-<< (a :all x :tl)
  (implies (and (all-<< x a) (ordered x))
    (ordered (append x (list a)))))

(property ordered-append-<<= (a :all x :tl)
  (implies (and (all-<<= a x) (ordered x))
    (ordered (append (list a) x))))

(property ordered-append (a :all x y :tl)
  (implies (and (all-<< x a) (ordered x) (all-<<= a y) (ordered y))
    (ordered (append x (cons a y)))))

(property all-<<-append (x :tl y :tl a :all z :all)
  (== (all-<< (append x (cons z y)) a)
      (and (all-<< x a) (all-<< y a) (<< z a))))

(property all-<<=-append (a :all x :tl y :tl z :all)
  (== (all-<<= a (append x (cons z y)))
      (and (all-<<= a x) (all-<<= a y) (<<= a z))))

(property all-<<-part1 (x :tl a :all z :all)
  (implies (all-<< x a)
      (all-<< (less z x) a)))

(property all-<<-part2 (x :tl a :all z :all)
  (implies (all-<< x a)
      (all-<< (notless z x) a)))
...
(property all-<<-qsort (x :tl a :all)
  (implies (all-<< x a)
           (all-<< (qsort x) a))
  :hints (("Goal" :induct (qsort x))))

(property all-<<=-qsort (a :all x :tl)
  (implies (all-<<= a x)
           (all-<<= a (qsort x)))
  :hints (("Goal" :induct (qsort x))))

(property ordered-insert (a :all x :tl)
  (implies (ordered x)
           (ordered (insert a x))))
...
(property isort-ordered-x=x (x :tl)
  (implies (ordered x)
    (== (isort x) x)))

(property ordered-implies-ordered-cdr (x2 :tl)
  (implies (ordered x2)
           (ordered (cdr x2))))

(property ordered-less-empty (x1 :all x2 :tl)
  (implies (and (consp x2)
                (ordered x2)
                (not (<< (car x2) x1)))
           (endp (less x1 x2))))

(property ordered-notless-id (x1 :all x2 :tl)
  (implies (and (consp x2)
                (ordered x2)
                (<<= x1 (car x2)))
           (== (notless x1 x2) x2)))

(property split-ordered (x1 :all x2 :tl)
  (implies (ordered x2)
           (== (append (less x1 x2) (notless x1 x2)) x2))
  :hints (("Subgoal *1/4.4'"
           :use ((:instance ordered-less-empty
                            (x1 (car x2))
                            (x2 (cdr x2)))))
          ("Subgoal *1/4.3'"
           :use ((:instance ordered-less-empty
...
(property split-isort (x1 :all x2 :tl)
  (implies (ordered x2)
    (== (insert x1 x2)
      (append (less x1 x2) (cons x1 (notless x1 x2)))))
  :hints (("Goal" :induct (insert x1 x2))
          ("Subgoal *1/2'"
           :use ((:instance ordered-less-empty
                            (x1 x1)
                            (x2 x2))))))

(property isort-qsort=isort (x :tl)
  (== (isort x)
      (isort (qsort x)))
  :hints (("Goal" :induct (qsort x))))

(in-theory (disable isort-qsort=isort isort-ordered-x=x))

(property isort-qsort=qsort (x :tl)
  (== (qsort x)
      (isort (qsort x))))
